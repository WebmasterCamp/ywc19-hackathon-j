...
const LotteryChart: FC<LotterChartProps> = ({
  lottoCart,
  setLottoCart,
  setHuayState,
}) => {
      ...
      <div>
        <div className="flex justify-between items-center">
          <div className="flex flex-col py-4 gap-2">
            <div className=" text-xl font-bold">รายการซื้อ</div>
            <div>งวดวันที่ 1 กรกฎาคม 2566</div>
          </div>
          <button
            className=" h-12 w-12 rounded-full bg-green-500 flex flex-col justify-center items-center"
            onClick={(e) => {
              setHuayState(0);
            }}
          >
        ...
        {lottoCart.length == 0 ? (
          <div className="flex justify-center mx-auto">
            <button
              className="py-4 focus:underline hover:underline"
              onClick={(e) => {
                setHuayState(0);
              }}
            >
        ...
        <div className="flex justify-between items-center my-4">
          <div className=" text-xl font-bold">รวมทั้งหมด</div>
          <div className=" text-xl font-bold">{95 * lottoCart.length} บาท</div>
        </div>
